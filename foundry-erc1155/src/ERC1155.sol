...
contract ERC1155 is IERC1155 {
    ...
    function setApprovalForAll(address operator, bool approved) external {
        ...
        emit ERC1155__ApproveForAll(msg.sender, operator, approved);
    }
    ...
    function safeTransferFrom(
        ...
        emit ERC1155__TransferSingle(msg.sender, from, to, id, value);
        if (to.code.length > 0) {
            require(
    ...
    function safeBatchTransferFrom(
        ...
        require(ids.length == values.length, "the length arrays do not match");
        for (uint i = 0; i < ids.length; i++) {
            balanceOf[from][ids[i]] -= values[i];
            balanceOf[to][ids[i]] += values[i];
        }
        emit ERC1155__TransferBatch(msg.sender, from, to, ids, values);
    ...
    function _mint(
        ...
        require(to != address(0), "to is zero address");
        balanceOf[to][id] += value;
        emit ERC1155__TransferSingle(msg.sender, address(0), to, id, value);
        if (to.code.length > 0) {
            require(
                IERC1155TokenReceiver(to).onERC1155Received(
                    msg.sender,
    ...
    function _batchMint(
        ...
        emit ERC1155__TransferBatch(msg.sender, address(0), to, ids, values);
    ...
    function _burn(address from, uint256 id, uint256 value) internal {
        ...
        balanceOf[from][id] -= value;
        emit ERC1155__TransferSingle(msg.sender, from, address(0), id, value);
    }
    ...
    function _batchBurn(
        ...
        require(from != address(0), "to is address zero");
        require(ids.length == values.length, "the length arrays do not match");
        for (uint i = 0; i < ids.length; i++) {
            balanceOf[from][ids[i]] -= values[i];
        }
        emit ERC1155__TransferBatch(msg.sender, from, address(0), ids, values);
    }
}
